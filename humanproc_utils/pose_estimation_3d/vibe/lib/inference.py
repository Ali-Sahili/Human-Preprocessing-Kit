...
from .models.vibe import VIBE_Demo
...
def vibe_infernce_video(args):
    ...
    elif args.tracking_method == 'yolopose':
        from .yolo_pose import yolo_inference
        tracking_results = yolo_inference(video_file, model_name="weights/yolo11x-pose.pt", 
                                            is_plot_poses = args.display)
    elif args.tracking_method == 'vitpose':
        from .vitpose.inference import vitpose_inference
        tracking_results = vitpose_inference(vid_file=video_file, 
                              model_name='weights/vitpose-b-multi-coco.pth', is_plot=args.display)
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        elif args.tracking_method == 'pose' or args.tracking_method == 'yolopose' or args.tracking_method == 'vitpose':
        ...
        with torch.no_grad():
            ...
            nb_joints = 25
            if args.tracking_method == 'yolopose' or args.tracking_method == 'vitpose':
                nb_joints = 17

            for batch in dataloader:
                if has_keypoints:
                    batch, nj2d = batch
                    norm_joints2d.append(nj2d.numpy().reshape(-1, nb_joints, 3))
